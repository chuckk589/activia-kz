<template>
  <div>
    ...
    :fields = "fields"
     ...
     itemsPerPageSelect
     >
      <template #credentials="{item}"><td>{{item.credentials || 'Не указано'}}</td></template>
      <template #promo="{item}"><td>{{item.promo || 'Не указано'}}</td></template>
      <template #registered="{item}"><td>{{item.registered?'Да':'Нет'}}</td></template>

      <template #locale="{item}"><td>{{locale(item.locale)}}</td></template>
      <template #role="{item}"><td>{{role(item.role)}}</td></template>
  ...
  computed:{
    getCurrentItems(){
      ...
      const csvCode = 'data:text/csv;charset=utf-8,'+ cols + '%0A' + encodeURIComponent(
        this.items.map((item) => `${item.id},${item.chat_id},${item.city?.ru},${item.credentials},${item.locale?.ru},${item.phone},${item.promo?.ru},${item.role?.ru},${item.registered?'Да':'Нет'},${new Date(item.createdAt).toLocaleString().replace(',','')}`)
        .join('\n')
      )
      return csvCode
    },
